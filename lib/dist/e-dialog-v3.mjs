...
import { getCurrentInstance, inject, ref, computed, unref, shallowRef, watchEffect, readonly, getCurrentScope, onScopeDispose, isRef, warn, defineComponent, openBlock, createElementBlock, mergeProps, renderSlot, createElementVNode, onMounted, onBeforeUnmount, provide, watch, nextTick, createBlock, Teleport as Teleport$1, useSlots, Text, resolveDynamicComponent, withCtx, Fragment, normalizeClass, createCommentVNode, reactive, toRef, createVNode, h, normalizeStyle, toDisplayString, Transition, withDirectives, createSlots, vShow, render } from "vue";
...
const index = {
  install(app, config = {}) {
    app.config.globalProperties.useDialog = function() {
      const vm = getCurrentInstance();
      return function(compName, props = {}, fn, selfConfig = {
        title: "标题",
        isConfirm: true
      }) {
        let AllConfig = Object.assign(config, selfConfig);
        const floorBtn = AllConfig.isConfirm ? [h(ElButton, {
          type: "primary",
          onClick: () => {
            success();
          }
        ...
        const dialogVnode = h(ElDialog, __spreadValues({
          onClosed: () => {
            close();
            dom.remove();
          },
